...
dry_run = False
factors = [1]
jobs = list(itertools.product(work_list, factors))


def train_scene(gpu, name, factor):
    dataset = "blender"
    if name[-1].isdigit():
        dataset = "Omnizl"

    output_dir = "output-no-op"
    op = 0
    sc = 0.1
    if not os.path.exists(
        os.path.join(output_dir, dataset, name, "octree-result.json")
    ):
        cmd = f"./general_train.sh {dataset} {output_dir} {name} {gpu} {op} {sc}"
        print(cmd)
        if not dry_run:
            os.system(cmd)

    output_dir = "output-no-sc"
    op = 3
    sc = 0
    if not os.path.exists(
        os.path.join(output_dir, dataset, name, "octree-result.json")
