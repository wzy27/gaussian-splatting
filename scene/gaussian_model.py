...
class GaussianModel:
    ...
    def get_normal(self):  # TODO: fix bug here
        ...
        normal_vec = (
            f.one_hot(min_scale_index, 3).unsqueeze(-1).type_as(rotation_matrix)
        )
        rotated_vec = rotation_matrix @ normal_vec
        return rotated_vec

    def get_xyz_by_opacity(self, threshold):
        opaque_mask = (self.get_opacity > threshold).squeeze()
        return self._xyz[opaque_mask]
    ...
    def extract_by_mask(self, mask, path):
        ...
        opacities = self.get_opacity[mask].detach().cpu().numpy()
        rgb = f_dc.copy()
