...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    first_epoch = 0
    epoch_bar = tqdm(range(first_epoch, args.epochs), desc="Overall Training progress")
    first_epoch += 1

    octree = LOctreeA.LOTLoad(path=os.path.join(dataset.source_path, "LOT_new_table_512.npz"),
                         path_d=None,
                         load_full=False,
                         load_dict=False,
                         device="cuda",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=dense_test_iter)
