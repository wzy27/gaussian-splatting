...
def training(
    ...
    render_camera_name = "r_95"
    while viewpoint_cam.image_name != render_camera_name:
        viewpoint_cam = viewpoint_stack.pop(1)

    render_path = os.path.join(dataset.source_path, "deform_render", render_camera_name)
    os.makedirs(render_path, exist_ok=True)

    total_cnt = gaussians.get_xyz.shape[0]
    base_dir = "data/Omni/dinosaur003/after_deform_info"
    position_dir = os.path.join(base_dir, "gaussian_new_poses")
    matrix_dir = os.path.join(base_dir, "gaussian_rotation_mat_files")

    progress_bar = tqdm(range(100), desc="Rendering")
    for i in range(100):
        position_path = os.path.join(position_dir, "{}_gaussian_new_pos.ply".format(i))
        matrix_path = os.path.join(
            matrix_dir, "{}_gaussian_rotation_mat_file.txt".format(i)
        )

        rotation_matrixs = load_rotation_matrix(matrix_path, total_cnt)
        gaussians.translate_by_ply(position_path)
        ...
        torchvision.utils.save_image(
            image, os.path.join(render_path, "frame_{}".format(i) + ".png")
        )
